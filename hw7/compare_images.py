...
autoencoder_name = 'best.h5'
encoder_name = 'encoder.h5'
seed = 40666888
...
#read dataset and tranform into a 40000*(32*32*3) array
    ...
    for n in range(40000):
        img = np.asarray( Image.open('../../ML_data/hw7/images/{:0>6d}.jpg'.format(n+1)) )
        dataset.append(img)
...
images_array = images_array[31:63]
...
K_result = KMeans(n_clusters=2, max_iter=5000, n_init=500, verbose=0, n_jobs=-1, random_state=seed).fit(PCA_data)
...
if(count<2500):
    count=5000-count
print('Dimension:', 64,' Accuracy: ', count/5000)
...
# pca = PCA(n_components=64, whiten=True, random_state=43095)
# pca.fit(encoded_images)
# PCA_data=pca.transform(encoded_images)
print('Shape After PCA: ',PCA_data.shape)
...
plt.scatter(x=PCA_data[A_as_B[0],0], y=PCA_data[A_as_B[0],1], color='blacku.', label='DatasetA as B')
