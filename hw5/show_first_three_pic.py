...
def testOnePicture (model, device, data, epsilon, lossFunc) :
    ...
    if count <= 2 :
        ori_np = output_prob_ori.cpu().data.numpy()
        att_np = output_prob_attack.cpu().data.numpy()
        np.save('ori_{}'.format(count),ori_np)
        np.save('att_{}'.format(count),att_np)
#        print(output_prob_ori)
#        print(type(output_prob_ori))
#        print('\n\n\n')
#        print(output_prob_attack)
    else :
        exit() 
