...
def parsingTrainingData(file_name) :
  ...
  np.save('both_3_mean',mean)
  np.save('both_3_std',std)
...
model.add(Conv2D(768,(5,5),input_shape = (48,48,1), activation = 'relu', padding='same',kernel_initializer=keras.initializers.RandomNormal(mean=0.0,stddev=0.05,seed=666)))
...
mcp = keras.callbacks.ModelCheckpoint('mcp-both-3-acc-{val_acc:.5f}.h5',
    monitor='val_acc',
    save_best_only=True,
    verbose=1,
    mode='auto',
...
# write result ###########################################################################
writeResult('both_3.csv',result)


# save model
model.save('both_3_m.h5')
...
np.save('both_3_val_loss',np_val_loss)
np.save('both_3_tra_loss',np_tra_loss)
np.save('both_3_val_acc',np_val_acc)
np.save('both_3_tra_acc',np_tra_acc)
print('end')
