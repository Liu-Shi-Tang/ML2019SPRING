...
mcp = keras.callbacks.ModelCheckpoint('mcp-aug-only-1-acc-{val_acc:.5f}.h5',
    monitor='val_acc',
    save_best_only=True,
    verbose=1,
    mode='auto',
...
history = model.fit_generator(datagen.flow(train_feature,train_label,batch_size=128),
...
np.save('aug_only_val_loss',np_val_loss)
np.save('aug_only_tra_loss',np_tra_loss)
np.save('aug_only_val_acc',np_val_acc)
np.save('aug_only_tra_acc',np_tra_acc)
print('end')
